...
describe("Create/Update Comments", async () => {
  ...
  test("Can post a comment", async () => {
    ...
    console.log(postCommentResult.insertedId);
    console.log( martianComments[0]._id);
...
console.log('result' , updateCommentResult.modifiedCount);
    expect(updateCommentResult.modifiedCount).toBe(1)
    // console.log(await MoviesDAO.getMovieByID(movieId));
    
    const martianComments = (await MoviesDAO.getMovieByID(movieId)).comments
    console.log('result' ,martianComments[0].text );
